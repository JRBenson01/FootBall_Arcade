...
Version:        4
Description:    Version 4 compartmentalizes the root of the game engine into its
                own file. Game elements can now be added via additional files.
"""
...
class gamePiece(object):
    ...
    def chase(self, piece):
        #print("Chasing", piece, "self.triggered:", self.triggered)
        if self.daze <= 0:
            ...
            self.daze -= 1
            #print(self.name, "dazed:", self.daze)
        self.move()
    ...
    def collision(self, collider):
        if self.hb_type == "circle":
            ...
            if self.triggered == True:
                ...
                #self.angle = angleRound(deg(math.atan(x_dist/y_dist)))
                #collider.angle = self.angle + 180
                self.daze = 100
                collider.daze = 100
